...
class Layer(_RawLayer):
    ...
    def as_bl(self, image = None):
        return self._psd._layer_as_bl(self._index, image)
...
class Group(_RawLayer):
    ...
    def as_bl(self, image = None):   
        ...
        layer_idxs = [layer._index for layer in layers]
        layer_idxs.reverse()
        if not image:
            name = self.name
        return  self._psd._layers_as_bl(layer_idxs, image, name)
...
def get_layers(layers,only_visible = True):
    "get layer list"
    layers_result = []
    for layer in layers: 
        if not layer.visible and only_visible:
            continue                 
        if layer.is_group:                
                layers_result += get_layers(layer.layers)
        else :
...
class PSDImage(object):
    ...
    def load(cls, path, encoding='utf8'):
        ...
        with open(path, 'rb') as fp:
            psd = cls.from_stream(fp, encoding)
            psd.file_path = path
            return psd
    ...
    def as_bl(self, image = None):
        """
        Returns a blender image for this PSD file.
        """       
        name = os.path.basename(self.file_path) if not image else None
        return bl_support.psd_to_bl_image(self.decoded_data, image, name)
    ...
    def _layers_as_bl(self, layer_idxs, image = None, name = None):  
        layers_rd =  self.decoded_data.layer_and_mask_data.layers.layer_records  
        button_idx = layer_idxs[0]
        if not name:
            name =layers_rd[button_idx].name if not image else None
        return bl_support.layers_to_bl_image(self.decoded_data, layer_idxs, name, image)

    def _layer_as_bl(self, index, image = None):
        return bl_support.layer_to_bl_image( self.decoded_data, index, image)
